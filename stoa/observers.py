...
def time_before(value, threshold):
    ...
    time1 = datetime.fromtimestamp(threshold)
    time2 = datetime.fromtimestamp(value)
...
class Logic(Node):
    def __init__(self, left=None, logic="then", right=None):
    ...
    def __str__(self):
        if self.logic is not "then":
            return "{0} {1} {2}".format(
                self.left, self.logic, self.right
            )
        else:
            return "{0}".format(self.left)
